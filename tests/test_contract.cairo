...
fn deploy_counter(initial_count: u32) -> (ICounterDispatcher, ICounterSafeDispatcher) {
    ...
    let safeDispacter = ICounterSafeDispatcher { contract_address };


    return (dispatcher, safeDispacter);
}
...
fn test_increase_counter() {
    ...
    assert(current_count == expected_count, 'Counter should increament by 1');
...
fn test_decrease_counter() {
    ...
    assert(current_count == expected_count, 'Counter should decreament by 1');
}
...
fn test_decrease_counter_underflow() {
    ...
    match safe_counter.decrease_counter() {
         ...
         Result::Err(panic_data) => { assert!(*panic_data[0] == NEGATIVE_COUNTER, "Should throw NEGATIVE COUNTER error");},
    }
}
...
fn test_reset_counter_non_owner() {
    ...
    match safe_counter.reset_counter() {
         Result::Ok(_) => panic!("non-owner cannot reset the connter"),
         Result::Err(panic_data) => {
            assert!(*panic_data[0] == 'Caller is not the owner', "Should throw error id caller is not the owner");
        },
    }

    let current_count = counter.get_counter();
    assert(current_count == initial_count, 'Counter should not reset');
}
...
fn test_reset_counter_as_owner() {
    ...
    let current_count = counter.get_counter();
    assert!(current_count == 0, "Counter should reset to 0");
}
